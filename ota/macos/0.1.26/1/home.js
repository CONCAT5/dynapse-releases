...
function popover(h, id, style) {
    ...
    if (id.startsWith("ai:")) {
        ...
        body = ready(a)
            ? `<div class="pop__row">${capChips(a.caps)}<button class="pill pill--soft pill--sm" data-act="logout-help" data-id="${a.key}">로그아웃 안내 ↗</button></div>`
...
const whereChip = (h) => h.where === "sync" ? `<span class="where where--sync">☁ 내 기기끼리 · 비공개</span>` : `<span class="where">◉ 이 PC에서만 · 올라가지 않음</span>`;
export function headerHtml(h) {
    ...
    return `<button class="brandmark" data-act="web" data-id="/" title="웹 열기"><img src="symbol.svg" alt="" class="top__logo" /><b>Dynapse</b></button>
...
export function assemblyHtml(mood = "qi") {
    ...
    <div class="asm__src">${card("googlegemini", "사진", `<img class="asm__photo" src="assembly/photo.jpg" alt="" />`, LAYOUT)}
      ${card("claude", "디자인", `<span class="asm__wire"><i></i><i></i><i></i></span>`, CAM)}</div>
