...
import { appVerify, backfill, importStart, ensureVerify, idleSessions, onChatMessage, openWeb, replayWaiting, sessions, syncAll } from "./chat.js";
...
export function deviceBody() {
    const t = (x, k) => ({ installed: x.installed, loggedIn: x.loggedIn, connected: x.connected, account: x.account ?? null, plan: x.plan ?? null, switching: switching[k] ?? null });
    return {
        device_id: S.deviceId, app_version: S.app?.version, ota: otaNow(), default_models: S.defaultModels,
        ...
        tools: { claude: t(S.tools.claude, "claude"), codex: t(S.tools.codex, "codex"),
            agy: { installed: S.ag.installed, loggedIn: S.ag.loggedIn, connected: null, account: S.ag.loggedIn ? "Google 계정" : null }, // agy는 계정 정보·로그아웃 명령이 없다(실측)
...
async function handleLink(raw) {
    ...
    if (action === "export") { // ~/Downloads zip(렌더 사진이 없으면 검증 1회로 만든 뒤)
        const kind = p.get("kind") === "html" ? "html" : p.get("kind") === "pdf" ? "pdf" : "png"; // PDF = 전체 페이지를 한 파일로(인쇄용)
        if (kind === "png") {
...
async function appAction(a) {
    ...
    if (a.name === "login" && a.model === "switch" && tool !== "gemini")
        await switchAccount(tool);
        ...
        await T().shell.open(INSTALL_URL[(tool === "gemini" ? "agy" : tool)]);
...
export function render() {
    ...
    $app.innerHTML = S.onboard ? onboardView() : mainHtml({ connected: !!S.token, hubBusy: S.hubBusy, version: a ? `${a.version}${otaNow() ? `.${otaNow()}` : ""}` : "", location: loc });
    if (logOpen)
...
(async () => {
    ...
    const autoWeb = () => {
        if (!S.token || S.onboard || document.visibilityState !== "visible" || Date.now() - goingWeb < 3000)
            return;
        goingWeb = Date.now();
        void T().core.invoke("go_web").catch(() => openWeb("/works"));
    };
